...
class CATPracticeEngine(PracticeEngineBase):
    ...
    def get_next_question(self, user, qTag, question_pool, session_store):
        this_engine = Assessment.objects.all().get(name='CAT Practice')

        # Rebuild session store
        if not session_store or session_store['engine'] != this_engine or session_store['qTag'] != qTag:
            session_store = {}
            session_store['engine'] = this_engine
            session_store['qTag'] = qTag
            session_store['numerator'] = 0
            session_store['denominator'] = 0
            session_store['ability'] = 0

            # Rebuild ability score from data
            responses = Response.objects.all().filter(user=user).filter(assessment=this_engine).filter(question__tag__exact=qTag)
            for prev_response in responses:
                session_store = self._compute_ability(session_store, prev_response.criterion, prev_response.correctness)

        # Get tested questions and omit them from question pool
        question_tested = Response.objects.all().filter(user=user).filter(assessment=this_engine).filter(question__tag__exact=qTag).values_list('question')
        question_pool = question_pool.exclude(id__in=question_tested)
    ...
    def get_user_ability(self, user, qTag):
        # Get user's ability from last attempted question
        response = Response.objects.all().filter(user=user).filter(assessment__name__exact='CAT Practice').filter(question__tag__exact=qTag).order_by('-date')
    ...
    def match_answers(self, user, response, question, session_store):
        ...
        #TODO use mark algo here
        mark=grade("ensemble",question.solution.content,response,question.mark)["score"]
        correctness=mark*1.0/question.mark
        ...
        # Rebuild session store
        if not session_store or session_store['engine'] != this_engine or session_store['qTag'] != question.tag:
            session_store = {}
            session_store['engine'] = this_engine
            session_store['qTag'] = question.tag
            session_store['numerator'] = 0
            session_store['denominator'] = 0
            session_store['ability'] = 0

            # Rebuild ability score from data
            responses = Response.objects.all().filter(user=user).filter(assessment=this_engine).filter(question__tag__exact=question.tag)
            for prev_response in responses:
                session_store = self._compute_ability(session_store, prev_response.criterion, prev_response.correctness)
        ...
        # Record response and ability
        new_response = Response(user=user, question=question, content=response, assessment=this_engine, mark=mark, criterion=difficulty, ability=normalised_ability)
        new_response.save()

        reply_vector = {'session_store': session_store, 'correctness': correctness, 'ability': normalised_ability,'mark':mark}
